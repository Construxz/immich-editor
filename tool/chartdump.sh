...
# SOURCE=photo.png renders that image instead of the chart; recipe name "optimize" = Optimize.kt.
export MSYS_NO_PATHCONV=1
A="$LOCALAPPDATA/Android/Sdk/platform-tools/adb.exe"
FILES=/sdcard/Android/data/io.github.construxz.photoeditor/files
OUT="$1"; shift; mkdir -p "$OUT"
SRC=
if [ -n "$SOURCE" ]; then
  "$A" -s emulator-5554 push "$SOURCE" "$FILES/" </dev/null >/dev/null
  SRC="-e source $(basename "$SOURCE")"
fi
while [ $# -gt 0 ]; do
  n="$1"; j="$2"; shift 2
  for t in 1 2 3; do
    "$A" -s emulator-5554 shell am instrument -w -e class io.github.construxz.photoeditor.ChartDump $SRC \
    ...
    "$A" -s emulator-5554 pull "$FILES/chart/$n.png" "$OUT/" </dev/null >/dev/null
